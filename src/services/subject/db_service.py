...
from src.db.models import SubjectDB
...
class SubjectDBService(BaseDBService):
    ...
    async def get_subject_by_id(self, *, subject_id: int) -> SubjectSchema:
        async with self.get_async_session() as sess:
            return await sess.scalar(select(SubjectDB).where(SubjectDB.id == subject_id))

    async def get_list_of_subjects(self) -> list[SubjectListItemSchema]:
        async with self.get_async_session() as sess:
            return await sess.scalars(select(SubjectDB))

    async def update_subject(self, *, subject_data: PatchSubjectSchema, subject_id: int) -> None:
        async with self.get_async_session() as sess:
            await sess.execute(update(SubjectDB).values(**subject_data.model_dump()).where(SubjectDB.id == subject_id))
            await sess.commit()
