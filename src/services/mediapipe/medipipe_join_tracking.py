...
from utils import colorBackgroundText, eyes_scale, FONT, lips_scale, head_scale, counter_scale
import time
...
def mediapipe_all(img, w, h, draw_all = False):
    ...
    with mp_face_mesh.FaceMesh(max_num_faces=2, min_detection_confidence=0.5, min_tracking_confidence=0.5) as face_mesh:
        ...
        keys = ['Отвод взгляда:', 'Открытие рта:', 'Больше одного человека:', 'Нет человека:', 'Голова отведена от центра:']
